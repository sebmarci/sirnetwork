...
$$\lambda_i = \beta \sum_j c_{ij} \frac{I_j(t)}{N_i}$$
...
- $\beta$: Infection rate
- $\mu$: recovery rate
- $c_{ij}$: Normalized social connectivity matrix. We remark that we use the convention $\sum_j c_{ij} = 1$ for all $i$. Then the effective contact rate matrix is $C_{ij} = \beta c_{ij}$. 
